...
public class CartService
{
    ...
    public void putPet(int id, int userId)
    {
        ...
        if (pet.getBody().length != 0) {cartDataAccessObject.addItem(id, pet.getBody()[0].getPrice(), userId);}
    }
    ...
    public void putStaff(int id, int userId)
    {
        RestTemplate restTemplate = new RestTemplate();
        HttpEntity httpEntity = new HttpEntity(headers);
        ResponseEntity<Staff[]> staff = restTemplate.exchange(config.get("Item") + "/stuff/" + id, HttpMethod.GET, httpEntity, Staff[].class);
    ...
    {
        int totalPrice = cartDataAccessObject.getTotalPrice(userId);
